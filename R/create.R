...
#' `"trestles"`, `"onofre"`, and `"jolla-ellessenne"`.
#' @examples
#' \dontrun{
#'
...
#' postcards::create_postcard(template = "jolla-ellessenne")
#' }
create_postcard <- function(file = "index.Rmd",
  ...
  if(create_image) {
    img_table <- as.list(
      system.file("img",
                  c("tobi.jpg", "xiang.jpg", "frank.jpg", "herzl.jpg", "artem.jpg"),
                  package = "postcards"))

    names(img_table) <- c("jolla", "jolla-blue", "trestles", "onofre", "jolla-ellessenne")
...
new_project_create_postcard <- function(path, ...) {
  ...
  template_table <- as.list(c("jolla", "jolla-blue", "trestles", "onofre", "jolla-ellessenne"))
  names(template_table) <- c("Jolla", "Jolla Blue", "Trestles", "Onofre", "Jolla by ellessenne")
